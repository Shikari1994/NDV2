...
/* ─── Тач-скролл под контролем JS ───
   ...
   normalizeScroll перехватывает тач и ведёт его тем же rAF-циклом, что и
   анимации, — рассинхрону взяться неоткуда. type: 'touch' здесь обязателен:
   по умолчанию GSAP забрал бы и колесо, а колесо уже ведёт Lenis, и два
   инерционных движка на одной оси дрались бы. allowNestedScroll оставляет
   рабочими прокручиваемые блоки внутри страницы.

   При reduced-motion не включаем: скролл-твинов там нет, синхронизировать
   нечего, а перехват жеста — сам по себе вмешательство в управление
   страницей, и на этом режиме оно тем более неуместно. */
if (!reduce && ScrollTrigger.isTouch) {
...
/* ─── Разворот «Ключевые цифры»: наезд колонок со сдвигом ───
   ...
   там скрыты, а кадры уходят фоном в сами карточки (CSS). */
{
  ...
  if (!reduce && shots.length > 1 && window.matchMedia('(min-width: 901px)').matches) {
    ...
    // шаг — высота липкой панели справа (100svh), а не окна: см. stickyH
    const step = () => stickyH(spread.querySelector('[data-spread-item]'))

    const seg = (i) => ({
      trigger: spread,
      start: () => 'top top-=' + (i - 1) * step(),
      end: () => '+=' + step() * 0.8,   // на 20% экрана раньше правого шва
      scrub: 0,
      invalidateOnRefresh: true,
    })
